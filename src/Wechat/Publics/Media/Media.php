...
class Media
{
    ...
    public function uploadimg($media_full_file_path)
    {
        $url = $this->WechatApiBaseURL.'cgi-bin/media/uploadimg?access_token='.$this->access_token;

        return $this->httpRespToArray($this->curlPostWxMedia($url, $media_full_file_path), ParamsRemark::MEDIA_UPLOADIMG_REQ, ParamsRemark::MEDIA_UPLOADIMG_RES);
